...
import com.fuelcell.ui.VariationAdapter;
...
public class CarProfileActivity extends NavActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		View contentView = inflater.inflate(R.layout.activity_car_profile,
				null, false);
		...
		planTrip.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Intent intentTrip = new Intent(CarProfileActivity.this,
		...
		ButtonSettings.setHomeButton(((ImageView) findViewById(R.id.mainicon)),
				mDrawer);
	...
	private void setSavedButtonDrawable(boolean isSaved) {
		if (isSaved) {
			save.setBackgroundDrawable(getResources().getDrawable(
					R.drawable.favourite_set_on_pressing));
		} else {
			save.setBackgroundDrawable(getResources().getDrawable(
					R.drawable.favourite_set_off_pressing));
		}
	}
	...
	protected void onStart() {
		...
		this.car = CarDatabase.obtain(CarProfileActivity.this).getCarFull(
				carFrame);
		...
		SharedPreferences defaultCarPrefs = getSharedPreferences("default",
				MODE_PRIVATE);
		...
		if ((defaultCarPrefs.getInt("year", 0) == car.year)
				&& defaultCarPrefs.getString("manufacturer", "not")
						.equalsIgnoreCase(car.manufacturer)
				&& defaultCarPrefs.getString("model", "not").equalsIgnoreCase(
						car.model)
				&& defaultCarPrefs.getString("vehicleClass", "not")
						.equalsIgnoreCase(car.vehicleClass)) {
			defaultButton.setBackgroundDrawable(getResources().getDrawable(
					R.drawable.is_default_unpress));
		} else {
			defaultButton.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					Toast.makeText(CarProfileActivity.this,
							"This car has been saved as the default car.",
							Toast.LENGTH_LONG).show();

					SharedPreferences defaultCarPrefs = getSharedPreferences(
							"default", MODE_PRIVATE);
					SharedPreferences.Editor prefEditor = defaultCarPrefs
							.edit();
					...
					defaultButton.setBackgroundDrawable(getResources()
							.getDrawable(R.drawable.is_default_unpress));
				}
			});
		}
		setOverflowClick();
		...
		List<Car> carList = CarDatabase.obtain(this).getCarsFull(carFrame);
		final Car[] variations = carList.toArray(new Car[carList.size()]);
		if (variations.length > 1) {
			builder.setAdapter(
					new VariationAdapter(this, R.layout.variations_list_item,
							variations),
					new DialogInterface.OnClickListener() {
	
						@Override
						public void onClick(DialogInterface dialog, int which) {
							setCar(variations[which]);
							car = variations[which];
						}
			...
			options.setBackgroundResource(R.drawable.options_button_pressed_dark);
		}
	}
	...
	private void setOverflowClick() {
		cylinderInfo.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Toast.makeText(CarProfileActivity.this,
						"Cylinders: " + cylinderInfo.getText(),
						Toast.LENGTH_LONG).show();
			}
		});
		engineInfo.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Toast.makeText(CarProfileActivity.this,
						"Engine Size: " + engineInfo.getText(),
						Toast.LENGTH_LONG).show();
			}
		});
		fuelInfo.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Toast.makeText(CarProfileActivity.this,
						"Fuel Type: " + fuelInfo.getText(), Toast.LENGTH_LONG)
						.show();
			}
		});
		...
		vehicleInfo.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Toast.makeText(CarProfileActivity.this,
						"Vehicle Type: " + vehicleInfo.getText(),
						Toast.LENGTH_LONG).show();
			}
		});
		efficiencyInfo.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Toast.makeText(CarProfileActivity.this,
						"Highway Fuel Efficiency: " + efficiencyInfo.getText(),
						Toast.LENGTH_LONG).show();
			}
		});
		emissionsInfo.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Toast.makeText(
						CarProfileActivity.this,
						"CO2 Emissions: " + emissionsInfo.getText() + "kg/year",
						Toast.LENGTH_LONG).show();
			}
		});
		gearInfo.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Toast.makeText(CarProfileActivity.this,
						"Gears: " + gearInfo.getText(), Toast.LENGTH_LONG)
						.show();
			}
		});
	}
	private void setCar(Car car){
		...
		engineInfo.setText(Float
				.toString((float) ((int) (car.engineSize * 100) / 100)));
		tranInfo.setText(car.transmission.toString());
		fuelInfo.setText(car.fuelType.toString());
		vehicleInfo.setText(car.vehicleClass);
		efficiencyInfo.setText(Float
				.toString((float) ((int) (car.highwayEffL * 100) / 100)));
